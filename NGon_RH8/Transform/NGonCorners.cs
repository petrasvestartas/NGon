    ...
    public class NGonCorners : GH_Component_NGon
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (m.Ngons.Count > 0) {
                ...
                bool[] f = m.GetNakedEdgePointStatus();

                for (int i = 0; i < m.TopologyVertices.Count; i++) {

                    if (!f[m.TopologyVertices.MeshVertexIndices(i)[0]]) continue;

                    if (m.TopologyVertices.ConnectedFaces(i).Length == 1) {
                        points.Add(m.TopologyVertices[i]);
                        mv.Add(m.TopologyVertices.MeshVertexIndices(i)[0]);
                        continue;
                    }

                    int[] e = m.TopologyVertices.ConnectedEdges(i);

                    List<int> vNaked = new List<int>(2);
                    if (e.Length >= 2) {


                        for (int j = 0; j < e.Length; j++) {
                            if (m.TopologyEdges.GetConnectedFaces(e[j]).Length == 1) {
                                var pair = m.TopologyEdges.GetTopologyVertices(e[j]);
                    ...
                    if (vNaked.Count == 2) {
                        Line l0 = new Line(m.TopologyVertices[i], m.TopologyVertices[vNaked[0]]);
                        Line l1 = new Line(m.TopologyVertices[i], m.TopologyVertices[vNaked[1]]);
