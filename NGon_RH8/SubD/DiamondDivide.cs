                ...
                if (DA.GetData(0, ref brep)) {
                    ...
                    if (brep.Faces.Count == 1 && !brep.Faces[0].IsClosed(0) && !brep.Faces[0].IsClosed(1)) {
                        ...
                        if (flag) {
                            ...
                            var mesh = NGonCore.MeshCreate.DiamondMesh(S, Math.Max(2,(int)U), V, T, F);
                        ...
                        if (!explode) {// && !flag2
                            ...
                            NGonCore.MeshCreate.HexOrDiamondGrid(brep, Math.Max(2, (int)U), (int)Math.Max(2, V), ref plinesClothed, ref plinesNaked, 0.5, false, feDivisions);
                            plinesClothed.AddRange(plinesNaked);
                            Mesh mesh = NGonCore.MeshCreate.MeshFromPolylines(plinesClothed, 0,1);
                            ...
                            for (int i = 0; i < brep.Faces.Count; i++) {
                                ...
                                S.SetDomain(0, new Interval(0, 1));
                                S.SetDomain(1, new Interval(0, 1));
                                var m = NGonCore.MeshCreate.DiamondMesh(S, Math.Max(2, (int)U), V, T, F);
                                mesh.Append(m);
                            }
