    ...
    public class Platonic : GH_Component_NGon {
        ...
        protected override void SolveInstance(IGH_DataAccess DA) {
            ...
            switch (id) {
                ...
                case (3):
                mesh = NGonCore.Tetrahedron.Create(r[0]);
                break;

                case (2):
                mesh = NGonCore.Cube.Create(r[0]);
                break;

                case (1):
                mesh = NGonCore.Torus.Create(s[0], s[Math.Min(1, s.Count - 1)], r[0], r[Math.Min(1, r.Count - 1)]);
                break;

                default :
                mesh = NGonCore.IcoSphere.Create(Math.Min(3,s[0]), (float)r[0]);
                break;
