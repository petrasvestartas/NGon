        ...
        public static List<Curve> SubdivideSurfaceIntoIsocurves(Surface S, int x, int y) {
            ...
            for (int i = 0; i < x + 1; i++) {
                ...
                for (int j = 1; j < y; j++) {
                    ...
                    if (i % 2 == 0)
                        t_ = (j % 2 == 0) ? NGonsCore.MathUtil.Lerp(t[j - 1], t[j + 1], T) : NGonsCore.MathUtil.Lerp(t[j - 1], t[j + 1], 1 - T);
                    else
                        t_ = (j % 2 == 0) ? NGonsCore.MathUtil.Lerp(t[j - 1], t[j + 1], 1 - T) : NGonsCore.MathUtil.Lerp(t[j - 1], t[j + 1], T);
