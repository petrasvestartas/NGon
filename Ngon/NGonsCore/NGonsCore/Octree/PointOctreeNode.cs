...
namespace NGonsCore.Octree {
    ...
    public class PointOctreeNode<T> where T : class {
        ...
        public PointOctreeNode(float baseLengthVal, float minSizeVal, Vector3d centerVal) {
        ...
        /// <returns></returns>
        public bool Add(T obj, Vector3d objPos) {
        ...
        /// <param name="objPos">Position of the object.</param>
        /// <returns>True if the object was removed successfully.</returns>
        public bool Remove(T obj, Vector3d objPos) {
            if (!Encapsulates(bounds, objPos)) {
                return false;
            }
        ...
        /// <param name="result">List result.</param>
        /// <returns>Objects within range.</returns>
        public void GetNearby(ref Vector3d position, ref float maxDistance, List<T> result) {
            // Does the node contain this position at all?
            // Note: Expanding the bounds is not exactly the same as a real distance check, but it's fast.
            // TODO: Does someone have a fast AND accurate formula to do this check?

            //bounds.Expand(new Vector3d(maxDistance * 2, maxDistance * 2, maxDistance * 2));

            //bounds.size = actualBoundsSize;
            bounds.Inflate(maxDistance * 2, maxDistance * 2, maxDistance * 2);
            bool contained = bounds.Contains((Point3d)position);
            ...
            // Check against any objects in this node
            for (int i = 0; i < objects.Count; i++) {
                if ((position - objects[i].Pos).Length <= maxDistance) {
                    result.Add(objects[i].Obj);
                }
            }
        ...
        public void DrawAllBounds(float depth = 0) {
            ...
            Bounds thisBounds = new Bounds(Center, new Vector3d(SideLength, SideLength, SideLength));
    ...
    */
        ...
        public PointOctreeNode<T> ShrinkIfPossible(float minLength) {
            ...
            if (children == null) {
                ...
                SetValues(SideLength / 2, minSize, (Vector3d)childBounds[bestFit].Center);
                return this;
            }
        ...
        */
        ...
        /// <param name="minSizeVal">Minimum size of nodes in this octree.</param>
        /// <param name="centerVal">Centre position of this node.</param>
        void SetValues(float baseLengthVal, float minSizeVal, Vector3d centerVal) {
            SideLength = baseLengthVal;
            minSize = minSizeVal;
            Center = centerVal;

            // Create the bounding box.
            actualBoundsSize = new Vector3d(SideLength, SideLength, SideLength);
            bounds = new BoundingBox((Point3d)Center, (Point3d)actualBoundsSize);
            ...
            childBounds[0] = new BoundingBox((Point3d)(Center + new Vector3d(-quarter, quarter, -quarter)), (Point3d)childActualSize);
            childBounds[1] = new BoundingBox((Point3d)(Center + new Vector3d(quarter, quarter, -quarter)), (Point3d)childActualSize);
            childBounds[2] = new BoundingBox((Point3d)(Center + new Vector3d(-quarter, quarter, quarter)), (Point3d)childActualSize);
            childBounds[3] = new BoundingBox((Point3d)(Center + new Vector3d(quarter, quarter, quarter)), (Point3d)childActualSize);
            childBounds[4] = new BoundingBox((Point3d)(Center + new Vector3d(-quarter, -quarter, -quarter)), (Point3d)childActualSize);
            childBounds[5] = new BoundingBox((Point3d)(Center + new Vector3d(quarter, -quarter, -quarter)), (Point3d)childActualSize);
            childBounds[6] = new BoundingBox((Point3d)(Center + new Vector3d(-quarter, -quarter, quarter)), (Point3d)childActualSize);
            childBounds[7] = new BoundingBox((Point3d)(Center + new Vector3d(quarter, -quarter, quarter)), (Point3d)childActualSize);
        }
        ...
        /// <param name="obj">Object to add.</param>
        /// <param name="objPos">Position of the object.</param>
        void SubAdd(T obj, Vector3d objPos) {
        ...
        /// <summary>
        /// Private counterpart to the public <see cref="Remove(T, Vector3d)"/> method.
        /// </summary>
        /// <param name="obj">Object to remove.</param>
        /// <param name="objPos">Position of the object.</param>
        /// <returns>True if the object was removed successfully.</returns>
        bool SubRemove(T obj, Vector3d objPos) {
            bool removed = false;
        ...
        void Split() {
            ...
            children[0] = new PointOctreeNode<T>(newLength, minSize, Center + new Vector3d(-quarter, quarter, -quarter));
            children[1] = new PointOctreeNode<T>(newLength, minSize, Center + new Vector3d(quarter, quarter, -quarter));
            children[2] = new PointOctreeNode<T>(newLength, minSize, Center + new Vector3d(-quarter, quarter, quarter));
            children[3] = new PointOctreeNode<T>(newLength, minSize, Center + new Vector3d(quarter, quarter, quarter));
            children[4] = new PointOctreeNode<T>(newLength, minSize, Center + new Vector3d(-quarter, -quarter, -quarter));
            children[5] = new PointOctreeNode<T>(newLength, minSize, Center + new Vector3d(quarter, -quarter, -quarter));
            children[6] = new PointOctreeNode<T>(newLength, minSize, Center + new Vector3d(-quarter, -quarter, quarter));
            children[7] = new PointOctreeNode<T>(newLength, minSize, Center + new Vector3d(quarter, -quarter, quarter));
        }
        ...
        static bool Encapsulates(BoundingBox outerBounds, Vector3d point) {
            return outerBounds.Contains((Point3d)point);
        }
        ...
        int BestFitChild(Vector3d objPos) {
        ...
        public static float SqrDistanceToRay(Ray3d ray, Vector3d point) {
            return (float)Vector3d.CrossProduct(ray.Direction, point - (Vector3d)ray.Position).SquareLength;//sqrMagnitude;
        }
    }
}