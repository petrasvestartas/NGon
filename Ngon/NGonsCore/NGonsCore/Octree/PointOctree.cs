...
namespace NGonsCore.Octree {
    ...
    public class PointOctree<T> where T : class {
        ...
        public PointOctree(float initialWorldSize, Vector3d initialWorldPos, float minNodeSize) {
        ...
        public void Add(T obj, Vector3d objPos) {
        ...
        public bool GetNearbyNonAlloc(Ray3d ray, float maxDistance, List<T> nearBy) {
            nearBy.Clear();
        ...
        /// <param name="maxDistance">Maximum distance from the ray to consider.</param>
        /// <returns>Objects within range.</returns>
        public T[] GetNearby(Vector3d position, float maxDistance) {
            List<T> collidingWith = new List<T>();
        ...
        void Grow(Vector3d direction) {
            ...
            for (int i = 0; i < 8; i++) {
                if (i == rootPos) {
                    ...
                    children[i] = new PointOctreeNode<T>(rootNode.SideLength, minSize, newCenter + new Vector3d(xDirection * half, yDirection * half, zDirection * half));
                }
            }
