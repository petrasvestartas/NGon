...
namespace NGonsCore {
    public static class MeshUtil {
        ...
        public static void GetOverlap(Mesh m0, Mesh m1,double tolerance = 0.1) {
            ...
            //1.Check if planes are coplaner
            //2. Get mesh pline boundingbox aligned to plane and check both planes bbox collision
        ...
        public static DataTree<Vector3d> insertionVectorsJoints(this Mesh M, bool Center = false, List<JointsVDAInputs> EV_ = null) {
            ...
            if (EV_ != null)
                if (EV_.Count == EV.BranchCount) {
                    DataTree<Vector3d> EV__ = new DataTree<Vector3d>();

                    for(int i = 0; i < EV_.Count; i++) {
                        EV__.Add(EV_[i].insertion, new GH_Path(EV_[i].meshEdge));
                    }
                    return EV__;
        ...
        public static List<Polyline> UnifyWinding(this Mesh m, int[][] fp)
        {
            ...
            for (int i = 0; i < fp.Length; i++)
            {
                ...
                int a = fp[i][0];
                int b = fp[i][1];
                Polyline p0 = polylines[a];
                Polyline p1 = polylines[b];
                ...
                //Shift polylines
                //int shift = Math.Abs(e0Local - e1Local);
                //p1.ShiftPolyline(shift);
                //fe[next] = NGonsCore.GrasshopperUtil.Shift(fe[next], shift);

                //Output
                //If history does not contain polylines add to polyline collections
            ...
            if (C0.BranchCount == C1.BranchCount)
            {
                ...
                for (int i = 0; i < C0.BranchCount; i++)
                {
                    ...
                    if (C0.Branch(i).Count == 1 && C1.Branch(i).Count == 1)
                    {
                        ...
                        int n = C1.Branch(i)[0].Count - 1;

                        bool closed = C0.Branch(i)[0].IsClosed && C1.Branch(i)[0].IsClosed;
                        if (!closed)
                            n++;
                            ...
                            //Create triangulated mesh from closed polyline
                            Mesh loft = new Mesh();
                        var mfList = new List<MeshFace>();
                        if (closed) {
                            //loft = MeshUtilSimple.StarMesh(C0.Branch(i)[0]);//Mesh.CreateFromClosedPolyline(C0.Branch(i)[0]);
                            //loft = Mesh.CreateFromClosedPolyline(C0.Branch(i)[0]);
                            var meshData = MeshUtilSimple.TriangulateOpenPolyline(C0.Branch(i)[0]);
                            loft = meshData.Item1;
                            mfList = meshData.Item2;
                            //loft = MeshCreate.MeshFromPolylines(new Polyline[] { C0.Branch(i)[0] }, 0, 1);
                        } else
                            for (int j = 0; j < n; j++) {
                                loft.Vertices.Add(C0.Branch(i)[0][j]);
                        ...
                        //var nid = loft.GetNakedVerticesID()[0];

                        //Rhino.RhinoApp.WriteLine(loft.Vertices.Count.ToString());
                        //Add bottom vertices
                        Polyline pline = C1.Branch(i)[0];
                        pline.Reverse();
                        for (int j = 0; j < n; j++)
                        {
                            
                            loft.Vertices.Add(C1.Branch(i)[0][j]);
                        }
                        ...
                        if (closed)
                        {
                            //loft.Vertices.Add(C1.Branch(i)[0].CenterPoint());
                            //n++;
                        ...
                        if (closed) {
                            ...
                            //loft.Vertices.ToPoint3dArray().Bake();
                            for (int j = 0; j < n-1; j++) {
    ...
    public static int[][] GetNGonsConnectedToNGonTopologyVertices(this Mesh mesh, HashSet<int> allNGonTv, bool flag = true) {
        ...
        static List<List<int>> allPointsFound;
        ...
        public static UInt32 Key(int i, int j) { return (UInt32)(i << 16 | j); }
        ...
        public static Mesh SimplifyMesh(this Mesh M, List<Point3d> fixedPoints) {
            ...
            Mesh simplifiedMesh = NGonsCore.MeshCreate.MeshFromPolylines(polylines, 0.0);
