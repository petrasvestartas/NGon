...
namespace NGonsCore.geometry3Sharp.mesh
{
    ...
    public class FaceGroupOptimizer
    {
        ...
        public NGonsCore.geometry3Sharp.mesh.DMesh3 Mesh { get { return mesh; } }
        ...
        public FaceGroupOptimizer(NGonsCore.geometry3Sharp.mesh.DMesh3 meshIn)
        {
            mesh = meshIn;
        ...
        public int DilateAllGroups(int nRings)
        {
            ...
            for (int k = 0; k < nRings; ++k) {
                ...
                foreach (int tid in triangles) {
                    ...
                    for (int j = 0; j < 3; ++j) {
                        if (nbr_tris[j] != NGonsCore.geometry3Sharp.mesh.DMesh3.InvalidID) {
                ...
                foreach (int tid in triangles) {
                    ...
                    if (bBackgroundOnly) {
                        for (int j = 0; j < 3 && bIsBorder == false; ++j) {
                            if (nbr_tris[j] != NGonsCore.geometry3Sharp.mesh.DMesh3.InvalidID && mesh.GetTriangleGroup(nbr_tris[j]) == BackgroundGroupID)
                                bIsBorder = true;
                        }
                    } else {
                        for (int j = 0; j < 3 && bIsBorder == false; ++j) {
                            if (nbr_tris[j] != NGonsCore.geometry3Sharp.mesh.DMesh3.InvalidID && mesh.GetTriangleGroup(nbr_tris[j]) != gid)
                                bIsBorder = true;
                        }
                    }
                    if (bIsBorder)
        ...
        private int find_max_nbr(int tid, out int nbr_same, out int nbr_diff, out int bdry_e)
        {
            ...
            for (int j = 0; j < 3; ++j) {
                ...
                nbr_g[j] = (nbr_t == NGonsCore.geometry3Sharp.mesh.DMesh3.InvalidID) ? -1 : mesh.GetTriangleGroup(nbr_tris[j]);
            }
            int max_idx = -1;
            for ( int j = 0; j < 3; ++j ) {
        ...
        private void count_same_nbrs(int tid, out int nbr_same, out int nbr_diff, out int nbr_bg, out int bdry_e)
        {
            ...
            for (int j = 0; j < 3; ++j) {
                int nbr_t = nbr_tris[j];
                if (nbr_t == NGonsCore.geometry3Sharp.mesh.DMesh3.InvalidID) {
                    bdry_e++;
                    continue;
                }
