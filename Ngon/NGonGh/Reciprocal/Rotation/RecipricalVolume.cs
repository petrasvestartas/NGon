...
namespace NGonGh.Utils {
    ...
    public class RecipricalVolume : GH_Component {
        ...
        protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager) {
            ...
            pManager.AddPlaneParameter("EndPlane0", "EndPlane0", "Plane0", GH_ParamAccess.list);
            pManager.AddPlaneParameter("EndPlane1", "EndPlane1", "Plane1", GH_ParamAccess.list);
            pManager.AddPlaneParameter("TopPlane", "TopPlane", "Plane0", GH_ParamAccess.list);
            pManager.AddPlaneParameter("BottomPlane", "BottomPlane", "Plane1", GH_ParamAccess.list);
            pManager.AddGenericParameter("Tenon", "Tenon", "Tenon", GH_ParamAccess.tree);
            pManager.AddGenericParameter("Mortise", "Mortise", "Mortise", GH_ParamAccess.tree);
        ...
        public Line[] GetLines(Line[] lines, Plane[] linePlanes, int[][] fe_,  out Plane[][] endPlanes,out int[][] neighbours, out Line[] linesCut, double move = 0) {
            ...
            for (i = 0; i < fe_.Length; i++) {
                ...
                for (int j = 0; j < fe_[i].Length; j++) {
                    ...
                    int current = fe_[i][j];
                    int prev = fe_[i][NGonsCore.MathUtil.Wrap(j - 1, fe_[i].Length)];
                    int next = fe_[i][NGonsCore.MathUtil.Wrap(j + 1, fe_[i].Length)];

                    //Rhino.RhinoApp.WriteLine(current.ToString());

                    //Rhino.RhinoApp.WriteLine(prev.ToString() + " " + next.ToString() + " " + current.ToString());

                    Point3d p0 = NGonsCore.PlaneUtil.LinePlane(linesMoved[current], linePlanes[prev]);
                    Point3d p1 = NGonsCore.PlaneUtil.LinePlane(linesMoved[current], linePlanes[next]);
