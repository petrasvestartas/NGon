    ...
    public class TheBox {
        ...
        public void ConstructPlanes() {
            ...
            for (int i = 0; i < p0.Count - 1; i++) {
                ...
                if (i > 0)
                    this.sidePlanesBi[i] = NGonsCore.PlaneUtil.BisectorPlane(sidePlanes[i - 1], sidePlanes[i]);
                if (i == p0.Count - 2)
                    this.sidePlanesBi[0] = NGonsCore.PlaneUtil.BisectorPlane(sidePlanes[sidePlanes.Length - 1], sidePlanes[0]);
        ...
        public void ConstructOutlines() {
            ...
            for (int i = 0; i < n; i++) {
                ...
                double angleNext = Vector3d.VectorAngle(sidePlanes[i].ZAxis, sidePlanes[NGonsCore.MathUtil.Wrap(i + 1, n)].ZAxis);
                double anglePrev = Vector3d.VectorAngle(sidePlanes[i].ZAxis, sidePlanes[NGonsCore.MathUtil.Wrap(i - 1, n)].ZAxis);
                ...
                Plane nextPlane = (isNextAngle && bisector) ? sidePlanesO[NGonsCore.MathUtil.Wrap(i + 1, n)] : sidePlanesBi[NGonsCore.MathUtil.Wrap(i + 1, n)];
                Plane prevPlane = (isPrevAngle && bisector) ? sidePlanes[NGonsCore.MathUtil.Wrap(i - 1, n)] : sidePlanesBi[NGonsCore.MathUtil.Wrap(i, n)];
            ...
            OutlineFromPlanes(sidePlanes[i], new Plane[]{ topPlane0, nextPlane,topPlane1, prevPlane })
                ...
                nextPlane = (isNextAngle && bisector) ? sidePlanes[NGonsCore.MathUtil.Wrap(i + 1, n)] : sidePlanesBi[NGonsCore.MathUtil.Wrap(i + 1, n)];
                prevPlane = (isPrevAngle && bisector) ? sidePlanesO[NGonsCore.MathUtil.Wrap(i - 1, n)] : sidePlanesBi[NGonsCore.MathUtil.Wrap(i, n)];
        ...
        public void ConstructJoints() {
            ...
            for (int i = 0; i < MidSrfPos.Count; i++) {
                ...
                for (int j = 0; j < s0.Length; j++) {
                    ...
                    int next = NGonsCore.MathUtil.Wrap(j + 1, sidePlanesR90.Length);
        ...
        private Polyline OutlineFromPlanes(Plane mainPlane, Plane[] planes) {
            ...
            for (int i = 0; i < planes.Length; i++) {
                Rhino.Geometry.Intersect.Intersection.PlanePlanePlane(mainPlane, planes[i], planes[NGonsCore.MathUtil.Wrap(i + 1, planes.Length)], out pt);
                outline.Add(pt);
            }
            outline.Close();
            return outline;
