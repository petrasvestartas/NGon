...
namespace NGonGh.SubD {
    public class CurveLoft : GH_Component_NGon {
        ...
        protected override void SolveInstance(IGH_DataAccess DA) {
            ...
            Curve CFirst = NGonsCore.Clipper.DataAccessHelper.Fetch<Curve>(DA, "CurveFirst");


            List<Curve> C = NGonsCore.Clipper.DataAccessHelper.FetchList<Curve>(DA, "Curves");
            int U = NGonsCore.Clipper.DataAccessHelper.Fetch<int>(DA, "DivisionsU");
            int V = NGonsCore.Clipper.DataAccessHelper.Fetch<int>(DA, "DivisionsV");
            if (CFirst != null ) {
                C.Insert(0, CFirst);
            }

            int divisions = -1;
            var meshes = NGonsCore.MeshCreate.MeshLoftMultiple(C, U,V, ref divisions);
