...
namespace NGonGh.SubD {
    public class CurveCut : GH_Component {
        ...
        protected override void SolveInstance(IGH_DataAccess DA) {
            ...
            Curve closedCurve = NGonsCore.Clipper.DataAccessHelper.Fetch<Curve>(DA, "ClosedCurve");
            List<Curve> C = NGonsCore.Clipper.DataAccessHelper.FetchList<Curve>(DA, "Curves");

            //NGonsCore.Geometry.RhinClip.CurveToPolyline(closedCurve);
            ...
            for (int i = 0; i < C.Count; i++) {
              ...
              //  NGonsCore.Geometry.Polyline3D.Boolean(NGonsCore.Clipper642.ClipType.ctDifference, x, CurvesToPolyline(cutters, dist);  closedCurve.ToPolyline(, Plane.WorldXY, 0.001, true);

                Curve cutCurve = NGonsCore.CurveUtil.BooleanOpenCurve(closedCurve, C[i], out result[i]);
                if (result[i] > 0) {
