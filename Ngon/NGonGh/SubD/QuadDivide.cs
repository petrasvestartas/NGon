                ...
                if (DA.GetData(0, ref brep)) {
                    if (brep.Faces.Count == 1 && flag2) {
                        ...
                        if (flag) {
                            ...
                            Mesh mesh = NGonsCore.MeshCreate.QuadMeshDistanceFromBrep(S, Math.Max(2, (int)U), Math.Max(2, (int)V), b, min, max, shift, ref distances);
                            ...
                            Mesh mesh = NGonsCore.MeshCreate.QuadMesh(S, Math.Max(2, (int)U), Math.Max(2, (int)V));
                            DA.SetData(0, mesh);
                        ...
                        if (!explode && foo) {//&& !flag2
                            ...
                            for (int i = 0; i < brep.Faces.Count; i++) {
                                ...
                                Mesh meshTemp = NGonsCore.MeshCreate.QuadMesh(surface, UDvisions, VDvisions, false, false, false);
                            ...
                            for (int i = 0; i < brep.Faces.Count; i++) {
                                ...
                                S.SetDomain(0, new Interval(0, 1));
                                S.SetDomain(1, new Interval(0, 1));
                                Mesh m = NGonsCore.MeshCreate.QuadMesh(S, Math.Max(2, (int)U), (int)Math.Max(2, V), false, false, false);
                                mesh.Append(m);
                            }
                            this.PreparePreview(mesh, DA.Iteration);
                            DA.SetData(0, mesh);
