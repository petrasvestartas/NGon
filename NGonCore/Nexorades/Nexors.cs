...
namespace NGonCore.Nexorades
{
    public class Nexors : IEnumerable<Nexor> {
        ...
        public Nexor this[int i] {
            get { return _nexors[i]; }
            set { this._nexors[i] = value.Duplicate(); }
        }
        ...
        public Tuple<DataTree<Polyline>,DataTree<Polyline>> BeamsFlatAndMembrane(NGonCore.MeshProps p, List<double> offsetDist) {


            DataTree<Polyline> dt = new DataTree<Polyline>();
            DataTree<Polyline> dtOutlines = new DataTree<Polyline>();
            DataTree<Polyline> dtEcc = new DataTree<Polyline>();
            ...
            try {
                ...
                for (int i = 0; i < p.M._countF(); i++) {
                    ...
                    for (int j = 0; j < p.M._countE(i); j++) {
                        ...
                        if (this[i, j].isNexor != 0) {
                            ...
                            this[i, j].endPl0 = (no[0] != -1) ? p.ePl[ne].ChangeOrigin(this[no[0], no[1]].line.Center()) : p.ePl[ne].ChangeOrigin(this[i, j].line.To);//change origin because planes are located at mesh edge
                            //Plane endPl0Mid = (no[0] == -1) ?  p.ePl[ne].ChangeOrigin(this[no[0], no[1]].line.Center()) :  p.ePl[ne].ChangeOrigin(this[i, j].line.To);//change origin because planes are located at mesh edge
                            ...
                            //Get outlines
                            if (offsetDist.Count != 5) continue;

                            Polyline poly = new Polyline(new[] { this[i, j].endPl0.Origin, this[i, j].ePl.Origin, this[i, j].endPl1.Origin });

                            double massAddition = 0;
                            Polyline Ecc0 = new Polyline();
                            Polyline Ecc1 = new Polyline();

                            for (int k = 1; k < offsetDist.Count; k += 2) {
                                double dist = this[i, j].direction ? 1 * offsetDist[k] : -1 * offsetDist[k + 1];

                                //reverse = k%2==1 ? reverse*-1 : reverse;
                                //massAddition *= reverse;

                                Point3d From = PlaneUtil.PlanePlanePlane(this[i, j].ePl, this[i, j].ePl90.MovePlanebyAxis(dist), this[i, j].endPl0);
                                Point3d To = PlaneUtil.PlanePlanePlane(this[i, j].ePl, this[i, j].ePl90.MovePlanebyAxis(dist), this[i, j].endPl1);
                                Line line = new Line(From, To);
                                if(this[i,j].isNexor!=-1)
                                    dt.Add(line.ToP(), new GH_Path(this[i, j].idNested));
                            ...
                            //Ecc0.Bake();
                            //Ecc1.Bake();
                            dtEcc.Add(Ecc0, new GH_Path(this[i, j].idNested));
                            dtEcc.Add(Ecc1, new GH_Path(this[i, j].idNested));
                            Point3d From_ = PlaneUtil.PlanePlanePlane(this[i, j].ePl, this[i, j].ePl90.MovePlanebyAxis(offsetDist[0]), this[i, j].endPl0);
                            Point3d To_ = PlaneUtil.PlanePlanePlane(this[i, j].ePl, this[i, j].ePl90.MovePlanebyAxis(offsetDist[0]), this[i, j].endPl1);
                            Line line_ = new Line(From_, To_);

                            dtOutlines.Add(line_.ToP(), new GH_Path(this[i, j].idNested));

                            //this[i, j].endPl1.Bake(100);//////////////////////////////
                            //this[i, j].endPl0.Bake(100);//////////////////////////////

                        }//is nexor
                    }//for j
                }//for i

                DataTree<Point3d> dtOutlinesCorrect = new DataTree<Point3d>();
                for (int i = 0; i < p.M._countF(); i++) {
                    ...
                    for (int j = 0; j < p.M._countE(i); j++) {
                        ...
                        if (this[i, j].isNexor != 0) {
                            ...
                            //Get current eccentricity points
                            Polyline pline = dtOutlines.Branch(new GH_Path(this[i, j].idNested))[0];
                            Polyline Ecc0 = dtEcc.Branch(new GH_Path(this[i, j].idNested))[0];
                            Polyline Ecc1 = dtEcc.Branch(new GH_Path(this[i, j].idNested))[1];

                            //pline.Bake();

                            Point3d p0 = pline.First;
                            Point3d p1 = pline.Last;
                            //dtOutlinesCorrect.Add(p0, new GH_Path(this[i, j].idNested));
                            //dtOutlinesCorrect.Add(p1, new GH_Path(this[i, j].idNested));
                            //pline.Bake();

                            Point3d p0_ = Point3d.Unset;
                            Point3d p1_ = Point3d.Unset;
                            ...
                            //Change current position and insert point to the other polyline

                            Point3d p0Next = Point3d.Unset;
                            bool flagp0Next = false;
                            if (no[0] != -1) {
                                //Rhino.RhinoApp.WriteLine(no[0].ToString() + " " + no[1].ToString());
                                if (!dtOutlines.PathExists(new GH_Path(no[0], no[1]))) continue;
                                Polyline plineNext = dtOutlines.Branch(new GH_Path(no[0], no[1]))[0];
                                //plineNext.Bake();

                                Line lNext = plineNext.ToLine();
                                Point3d p00 = LineUtil.LineLine(Ecc0.ToLine(), lNext);
                                p0_ = p00;
                                Point3d p01 = LineUtil.LineLine(Ecc1.ToLine(), lNext);
                                flagp0Next = true;
                                //p00.Bake();
                                //p01.Bake();
                                //if (p00 == Point3d.Unset) {
                                //    p1_ = p01;
                                //} else if (p01 == Point3d.Unset) {
                                //    p0_ = p00;
                                //} else {
                                //    if (lNext.ClosestPoint(p00, false).DistanceToSquared(p00) < lNext.ClosestPoint(p01, false).DistanceToSquared(p01))
                                //        p0_ = p00;
                                //    else
                                //        p1_ = p01;
                                //}
                            ...
                            int pe = E[fe.Prev(j)];//get previous ngon edge
                            int[] po = p.M._OppositeFE(i, (j - 1).Wrap(p.M._countE(i)));////get prev opposite edge
                            bool flagp0Prev = false;
                            Point3d p0Prev = Point3d.Unset;
                            if (po[0] != -1) {
                                if (!dtOutlines.PathExists(new GH_Path(po[0], po[1]))) continue;
                                Polyline plinePrev = dtOutlines.Branch(new GH_Path(po[0], po[1]))[0];
                                flagp0Prev = true;
                                Line lPrev = plinePrev.ToLine();
                                Point3d p00 = LineUtil.LineLine(Ecc0.ToLine(), lPrev);
                                Point3d p01 = LineUtil.LineLine(Ecc1.ToLine(), lPrev);
                                p1_ = p01;
                                //if (p00 == Point3d.Unset) {
                                //    p1_ = p01;
                                //} else if (p01 == Point3d.Unset) {
                                //    p0_ = p00;
                                //} else {
                                //    if (lPrev.ClosestPoint(p00, false).DistanceToSquared(p00) < lPrev.ClosestPoint(p01, false).DistanceToSquared(p01)) {
                                //        p0_ = p00;
                                //        Rhino.RhinoApp.WriteLine("B 0");
                                //    } else {
                                //        p1_ = p01;
                                //        Rhino.RhinoApp.WriteLine("B 1");
                                //    }
                                //}
                            }

                            Point3d p0Mid = (p0 + p0_) * 0.5;
                            Point3d p1Mid = (p1 + p1_) * 0.5;
                            //p0Mid.Bake();
                            //p1Mid.Bake();
                            ...
                            if (flagp0Next) {
                                ...
                                dtOutlinesCorrect.Add(p0Mid, new GH_Path(this[i, j].idNested));
                                dtOutlinesCorrect.Add(p0Mid, new GH_Path(this[no[0], no[1]].idNested));
                            } else {
                                dtOutlinesCorrect.Add(p0, new GH_Path(this[i, j].idNested));
                            }
                            if (flagp0Prev) {
                                dtOutlinesCorrect.Add(p1Mid, new GH_Path(this[i, j].idNested));
                                dtOutlinesCorrect.Add(p1Mid, new GH_Path(this[po[0], po[1]].idNested));
                            } else {
                                dtOutlinesCorrect.Add(p1, new GH_Path(this[i, j].idNested));
                            }
                            //dtOutlinesCorrect.Add(p1Mid, new GH_Path(this[i, j].idNested));
                ...
                DataTree<Polyline> dtPlines = new DataTree<Polyline>();
                for (int i = 0; i < dtOutlinesCorrect.BranchCount; i++) {
                    if (dtOutlinesCorrect.Branch(i).Count > 0) {
                        if(dtOutlinesCorrect.Branch(i).Count<2) continue;
                        dtPlines.Add(PolylineUtil.FitPline(dtOutlinesCorrect.Branch(i)), new GH_Path(dtOutlinesCorrect.Path(i).Indices[0]));
                        //Rhino.RhinoApp.WriteLine(dtOutlinesCorrect.Branch(i).Count.ToString());
                        //PolylineUtil.FitPline(dtOutlinesCorrect.Branch(i)).Bake();
                    }
                }
                ...
                DataTree<Polyline> dtPlinesWindows = new DataTree<Polyline>();

               for (int i = 0; i < dtPlines.BranchCount; i++) {
                    if (dtPlines.Branch(i).Count == 2) {
                      Polyline pline0 = dtPlines.Branch(i)[0].Duplicate();
                       Polyline pline1 = dtPlines.Branch(i)[1].Duplicate();
                        if (pline0.Last.DistanceToSquared(pline1.First) > pline0.Last.DistanceToSquared(pline1.Last))
                            pline1.Reverse();
                        pline0.AddRange(pline1);
                        //pline0.Bake();

                        //        //pline0.CollapseShortSegments(0.01);
                        //        //pline1.CollapseShortSegments(0.01);

                        //        //Rhino.RhinoApp.WriteLine(dtPlines.Branch(i)[0].Count.ToString());
                        //        //Rhino.RhinoApp.Write(dtPlines.Branch(i)[1].Count.ToString());


                        if (pline0.Count == 8) {

                            Polyline plineA = new Polyline(new Point3d[] { pline0[0], pline0[1], pline0[6], pline0[7], pline0[0] });
                            Polyline plineB = new Polyline(new Point3d[] { pline0[1], pline0[2], pline0[5], pline0[6], pline0[1] });
                            Polyline plineC = new Polyline(new Point3d[] { pline0[2], pline0[3], pline0[4], pline0[5], pline0[2] });

                            dtPlinesWindows.Add(plineA, new GH_Path(dt.Path(i).Indices[0]));
                            dtPlinesWindows.Add(plineB, new GH_Path(dt.Path(i).Indices[0]));
                            dtPlinesWindows.Add(plineC, new GH_Path(dt.Path(i).Indices[0]));
                            //plineA.Bake();
                            //plineB.Bake();
                            //plineC.Bake();
                            //}else if (dtPlines.Branch(i)[1].Count == 4 && dtPlines.Branch(i)[0].Count==2) {



                            //Polyline plineA = new Polyline(new Point3d[] { pline0[0], pline0[1], pline1.ClosestPoint(pline0[1]), pline1[1], pline0[0] });
                            //Polyline plineB = new Polyline(new Point3d[] { pline0[1], pline0[2], pline0[5], pline0[6], pline0[1] });
                            //Polyline plineC = new Polyline(new Point3d[] { pline0[2], pline0[3], pline0[4], pline0[5], pline0[2] });
                            ...
                            pline0.Close();
                            dtPlinesWindows.Add(pline0, new GH_Path(dt.Path(i).Indices[0]));
                           

                        }
                    }
                }
                return Tuple.Create(dt, dtPlinesWindows);
            } catch (Exception e) {
                Rhino.RhinoApp.WriteLine(e.ToString());
            }
        ...
        public DataTree<Polyline> Beams2(NGonCore.MeshProps p)
        {
        ...
        public DataTree<Polyline> Panels(NGonCore.MeshProps p)
        {
            //Draw panels
        ...
        /// <param name="p"></param>
        /// <returns></returns>
        public DataTree<Polyline> RoundBeams(NGonCore.MeshProps p, double radius = 0.075)
        {
                ...
                for (int j = 0; j < p.M._countE(i); j++)
                {
                    ...
                    if (this[i, j].isNexor != 0)
                    {
                        ...
                        if (this[on[0], on[1]].isNexor != 0 && this[op[0], op[1]].isNexor != 0)
                        {
                            ...
                            this[i, j].pipe = NGonCore.PipeUtil.CreatePipe(
                                line,
        ...
        public DataTree<Line> GetNexorLines(int naked = 1,int u_1v1Both0 = 0)
        {
            DataTree<Line> dt = new DataTree<Line>();
            ...
            for (int i = 0; i < this._nexors.Count; i++)
            {
                ...
                {
                    if(u_1v1Both0!=0)
                        if (this._nexors[i].direction == (u_1v1Both0==-1)) continue;
                    Line line = this._nexors[i].line;
                    dt.Add(line, new Grasshopper.Kernel.Data.GH_Path(this.id_[i].Item1, this.id_[i].Item2));
                }
            }
            return dt;
        }

        public DataTree<Polyline> GetNexorPlines(int naked = 1) {
            //DataTree<Line> dt = new DataTree<Line>();
            DataTree<Point3d> dt = new DataTree<Point3d>();

            for (int i = 0; i < this._nexors.Count; i++) {
                if (this._nexors[i].isNexor == naked) {
                    for(int j = 0; j < this._nexors[i].ecc.Count; j++) {
                        Point3d p = this._nexors[i].ecc[j].PointAt(0.5);
                        int nextID = this._nexors[i].eccID[j];
                        if (nextID == -1) continue;
                       dt.Add(p, new Grasshopper.Kernel.Data.GH_Path(this.id_[i].Item1, this.id_[i].Item2));
                        if (this._nexors[nextID].isNexor != naked) continue;
                        dt.Add(p, new Grasshopper.Kernel.Data.GH_Path(this.id_[nextID].Item1, this.id_[nextID].Item2));
                    }
                }
            }

            DataTree<Polyline> dtPlines = new DataTree<Polyline>();
            for (int i = 0; i < dt.BranchCount; i++) {
                if (dt.Branch(i).Count > 0) {
                    dtPlines.Add(PolylineUtil.FitPline(dt.Branch(i)), new GH_Path(dt.Path(i).Indices[0]) );
                }
            }
        ...
        public DataTree<Plane> GetNexorCenterPlanes(int naked = 1) {
