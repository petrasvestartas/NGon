...
namespace NGonCore.Octree {
	...
	readonly Queue<Ray> lastRayCollisionChecks = new Queue<Ray>();
        ...
        public BoundsOctree(float initialWorldSize, Vector3d initialWorldPos, float minNodeSize, float loosenessVal) {
            ...
            //looseness = looseness.Clamp(1.0f, 2.0f);
            looseness = MathUtil.Clamp(loosenessVal, 1.0f, 2.0f);
        ...
        public void Add(T obj, BoundingBox objBounds) {
            ...
            while (!rootNode.Add(obj, objBounds)) {
                Grow((Vector3d)objBounds.Center - rootNode.Center);
                if (++count > 20) {
        ...
        /// <returns>True if there was a collision.</returns>
        public bool IsColliding(Ray3d checkRay, float maxDistance) {
            //#if UNITY_EDITOR
            // For debugging
