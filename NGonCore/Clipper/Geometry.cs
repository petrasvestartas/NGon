...
namespace NGonCore.Geometry {
    ...
    public static class RhinClip {
        ...
        public static Tuple<List<Polyline>,List<Polyline>,List<Polyline>> BooleanIntersection(List<Curve> curves, List<Curve> cutters, double dist, double scale =1e10) {
            ...
            //  A = NGonCore.Geometry.Polyline3D.Boolean(ClipType.ctIntersection, polylines, cutters, Plane.WorldXY, 1 / 1e10, true);
            ...
            for (int j = 0; j < curves.Count; j++) {
                ...
                NGonCore.Clipper642.Clipper clipper = new NGonCore.Clipper642.Clipper();
                ...
                foreach(IntPoint intpt in input){
                  NGonCore.Clipper642.Clipper.PointInPolygon(intpt, cutter);
                }
           */
                ...
                solution = NGonCore.Clipper642.Clipper.SimplifyPolygons(solution, PolyFillType.pftEvenOdd);
        ...
        public static Polyline SimplifyCurve(Curve C, double scale = 1e10) {
            ...
            var input = PolylineToIntPoint(polyline, scale);
            var solution = NGonCore.Clipper642.Clipper.SimplifyPolygon(input, PolyFillType.pftEvenOdd);
        ...
        public static bool IsPointInside(this Polyline polyline_, Point3d p_, double scale = 1e10, double tol = 0.001)
        {
            ...
            //Rhino.RhinoApp.WriteLine(NGonCore.Clipper642.Clipper.PointInPolygon(inputP, input).ToString());
           bool isInside = NGonCore.Clipper642.Clipper.PointInPolygon(inputP, input)!=0 ;
    ...
    public static class Polyline3D {
        ...
        public static void Offset(IEnumerable<Polyline> polylines, double distance, out List<Polyline> outContour,
          out List<Polyline> outHole) {
            ...
            //
            // ReSharper disable once PossibleMultipleEnumeration
            Offset(polylines, OpenFilletType.Square, ClosedFilletType.Miter, distance, pln, RhinoDoc.ActiveDoc.ModelAbsoluteTolerance, out outContour, out outHole);
        }
        ...
        public static Polyline[] Offset(this IEnumerable<Polyline> polylines, double distance) {
            ...
            if (distance != 0) {
                ...
                foreach(Polyline p in polylines) { 
                    ...
                    NGonCore.Geometry.Polyline3D.Offset(p, Math.Abs(distance), out c0, out c1);
        ...
        public static int IsInside(this Polyline polyline, Point3d point, Plane pln, Double tolerance) {
            return NGonCore.Clipper642.Clipper.PointInPolygon(point.ToIntPoint2D(pln, tolerance), polyline.ToPath2D(pln, tolerance));
        }
          ...
          Plane pln, double tolerance, bool evenOddFilling, double disregardPolygonsWithArea = 0.01) {
            ...
            foreach (var pline0 in polyA) {
                ...
                foreach (var pline1 in polyB) {
                    if (pline1.plane().ClosestPoint(pline0[0]).DistanceToSquared(pline0[0]) > 0.001) {
            ...
            foreach (Polyline plA in polyA) {
                ...
                if (count == 0) {
                    area0 = Math.Abs(NGonCore.Clipper642.Clipper.Area(p0) * tolerance * tolerance);
                    count++;
                }
            }
            ...
            foreach (Polyline plB in polyB) {
                ...
                if (count == 1) {
                    area1 = Math.Abs(NGonCore.Clipper642.Clipper.Area(p0) * tolerance * tolerance);
                    count++;
                }
            }
            ...
            foreach (PolyNode pn in polytree.Iterate()) {
                if (pn.Contour.Count > 1) {
                    if (disregardPolygonsWithArea > 0)
                    {
                        ...
                        double area = Math.Abs(NGonCore.Clipper642.Clipper.Area(pn.Contour)*tolerance* tolerance);
