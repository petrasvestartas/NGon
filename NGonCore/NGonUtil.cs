...
namespace NGonCore
{
...

    {
        ...
        public Dictionary<int, int> faceColors;
        ...
        public MeshProps(Mesh M)
        {
            ...
            this.faceColors = Algo.Graph._GraphColorFacesDict(M);
        ...
        public Line[][] NexorTranslateLines(ref NGonCore.Nexorades.Nexors nexors, double[][] D_, bool scale = false)
        {
            return NexorTranslateLines(this.EFLines, ref nexors, this.M, this.edgeColors, this.FEFlatten, D_, scale);
        }


        public Line[][] NexorTranslateLines(Line[][] EFLines, ref NGonCore.Nexorades.Nexors nexors, Mesh M, Dictionary<int, int> edgeColors, int[][] FEFlatten, double[][] D_, bool scale = false)
        {

            double[][] D = new double[EFLines.Length][];

            Line[][] EFLinesCopy = new Line[EFLines.Length][];
            int[][] EFLinesFlag = new int[EFLines.Length][];

            for (int i = 0; i < EFLines.Length; i++)
            {
                EFLinesCopy[i] = new Line[EFLines[i].Length];
                EFLinesFlag[i] = new int[EFLines[i].Length];
            ...
            for (int i = 0; i < M._countF(); i++)
            {
                ...
                for (int j = 0; j < FEFlatten[i].Length; j++)
                {
                    ...
                    //bool flag = RowColumn ? j % 2 == 0 : j % 2 != 0;
            ...
            //        if (nexors[i, j].direction)
            //            EFLinesCopy[i][j] = Line.Unset;
