    ...
    public static class MeshCreate {
            ...
            try {
                ...
                for (int i = 0; i < brep.Faces.Count; i++) {
                    ...
                    if (HexOrDiamond && t != 0) {
                        ...
                        if (feDivisions == null)
                            mesh = NGonCore.MeshCreate.HexMesh(surface, udiv, vdiv, t, 0, ref distanceA, ref distanceB, true, false);
                        else
                        {
                            mesh = NGonCore.MeshCreate.HexMesh(surface, udiv, vdiv, t, 0, ref distanceA, ref distanceB, true, false);
                        ...
                        if (feDivisions == null)
                            mesh = NGonCore.MeshCreate.DiamondMesh(surface, udiv, vdiv, 0, false);
                        else
                            mesh = NGonCore.MeshCreate.DiamondMesh(surface, udiv, vdiv, 0, false);
                    ...
                    for (int j = 0; j < mesh.Ngons.Count; j++) {
                        ...
                        long NGonID = NGonCore.MathUtil.GetKey(i, j);
                ...
                bool hasEdges = false;
                NGonCore.Graphs.UndirectedWeightedGraph g = new NGonCore.Graphs.UndirectedWeightedGraph(dict_NGonID_Polyline.Count);
               ...
               //Print(hasEdges.ToString());
                ...
                if (hasEdges) {
                    List<NGonCore.Graphs.UndirectedWeightedGraph> cc = g.ConnectedComponents(g, false);
        ...
        public static List<Mesh> MeshLoftMultiple(List<Curve> C, int U, int V, ref int divisions)
        {
            ...
            if (C.Count > 1)
            {
                ...
                {
                    Mesh mesh = NGonCore.MeshCreate.MeshLoft(linesSets[i].ToList(), U);
                    meshes.Add(mesh);
                }
                return meshes;
            }
        ...
        public static Mesh MeshFromPolylines(IEnumerable<Polyline> nurbsCurves, double weld, int triangulation = 0) {
            ...
            foreach (Polyline polyline in nurbsCurves) {
                ...
                switch (switchVar) {
                    ...
                    Mesh temp = switchVar<0 ? MeshUtilSimple.TriangulateOpenPolyline(p,out reverse).Item1 : Mesh.CreateFromClosedPolyline(p);
                   // temp.Bake();
        ...
        public static Mesh MeshFromPolygonalBrep(this Brep brep, double weld, ref Polyline[] outlines) {
            ...
            return NGonCore.MeshCreate.MeshFromPolylines(outlines, weld,-1);
        ...
        public static Rhino.Geometry.Mesh LoftPolylineWithHoles(List<Polyline> outlines0, List<Polyline> outlines1, bool addNGons = false) {
            ...
            try {
                ...
                int[] cpID = CP.RTreeSearchIDOnlyOne(ptsPlines0, ptsMesh, 0.01); //use dictionary search istead
        ...
        public  static  Tuple<DataTree<Polyline>, DataTree<Polyline>> Stellate(Mesh m, List<double> d0, List<double> d1) {
            ...
            for (int i = 0; i < plines.Length; i++) {
                ...
                for (int j = 0; j < plines[i].Count - 1; j++) {
                    Point3d p = NGonCore.PlaneUtil.LinePlane(line, plane.ChangeOrigin(plines[i][j]));
                ...
                for (int j = 0; j < plines[i].Count - 1; j++) {
                    ...
                    Point3d p = NGonCore.PlaneUtil.LinePlane(lines[j], plane);
